...
_BLANK_METADATA = """\
// metadata.typ — document metadata for the {name} template
// Edit these values before compiling your document.
...
_BLANK_TEMPLATE = """\
// template.typ — page layout and styles for the {name} template
#import "metadata.typ": title, author, date
...
_BLANK_MAIN = """\
// main.typ — entry point for the {name} template
#import "template.typ": template
#import "metadata.typ": title, author, date, abstract
...
This is a sample document generated by the *{name}* template. Edit
`metadata.typ` to update the title, author, and date. Edit `template.typ` to
customise the page layout and typographic styles.

Add your content below. References are managed in `sources.bib` — cite them
with `#cite(<key>)`, for example: typst-manager is distributed via PyPI
@typst-manager.
...
class TemplateStore:
    ...
    def create_blank(self, name: str, description: str = "") -> Template:
        """
        Create a new template with boilerplate files:
          - main.typ       entry point (imports the others)
          - template.typ   page layout and styles
        ...
        ensure_dir(dest)
        (dest / "metadata.typ").write_text(
            _BLANK_METADATA.format(name=name), encoding="utf-8"
        )
        (dest / "template.typ").write_text(
            _BLANK_TEMPLATE.format(name=name), encoding="utf-8"
        )
        (dest / "sources.bib").write_text(
            _BLANK_BIB.format(name=name), encoding="utf-8"
        )
    ...
    def create_from_dir(self, name: str, source_dir: Path, description: str = "") -> Template:
        """
        Create a new template by importing an existing document directory.
        All files are copied except compiled artefacts (.pdf) and any
        pre-existing meta.toml (a fresh one is written instead).
        Raises FileNotFoundError if source_dir does not exist.
        Raises ValueError if source_dir contains no main.typ.
        """
        name = _validate_name(name)
        ...
        if not (source_dir / "main.typ").exists():
            raise ValueError(
                f"'{source_dir}' does not contain a main.typ file. "
                "Make sure you point to the root of a Typst document project."
            )
        ...
        for src in source_dir.rglob("*"):
            ...
            if src.name in _EXCLUDED_NAMES or src.suffix.lower() in _EXCLUDED_SUFFIXES:
                continue
            dst = dest / rel
